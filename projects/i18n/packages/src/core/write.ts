...
const objectUpdate = async (ctx: {
  ...
  if (ctx.templateFile) {
    ...
    template = JSON.parse(ctx.templateString)
  }
  ...
  if (Object.keys(exportJSONData).length > 0) {
    ...
    if (ctx.templateString) {
      const obj = JSON.parse(ctx.templateString)
        ...
        templateFiles.forEach((tt) => {
          ...
          if (!fileNames.includes(exportWithTemplate)) {
            ...
            objectUpdate({
              ...
              templateFile: tt,
              directoryAsNamespace: ctx.options.directoryAsNamespace,
              directory: true,
            })
          }
        })
      ...
      languages.forEach((lang) => {
        ...
        */
        try {
           ...
           */
            if (ctx.options.directoryAsNamespace) {
          ...
          // isExist directory
            ...
            objectUpdate({
              exportFile,
              templateString: JSON.stringify(obj, null, 2),
              directoryAsNamespace: ctx.options.directoryAsNamespace,
            })
          })
